...
class WordNetServiceTest extends FlatSpec with Matchers with BeforeAndAfter {
  ...
  "Plethora of fish at sea" should "have synset options for 3 words" in {
    val options = WordNetService.lookupOptions("Plethora of fish at sea")
  ...
  "Lots of fish near the river bank" should "have synset options for 2 words after compoundification" in {
    val options = WordNetService.lookupOptions("Lots of fish near the river bank")
  ...
  "Going to deposit some money at the bank" should "have synset options for 3 words" in {
    val options = WordNetService.lookupOptions("Going to deposit some money at the bank")
    options.size shouldEqual(3)
  ...
  "New York" should "be compounded" in {
    val options = WordNetService.lookupOptions("You live in New York")
    options.size shouldEqual(2)
  ...
  "A large sea of people" should "all steps" in {
    val options = WordNetService.lookupOptions("A large sea of people")
    val result = Neo4JGraphService.disambiguate(options)
